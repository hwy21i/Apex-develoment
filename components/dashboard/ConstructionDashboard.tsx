...
function Kpi({ label, value, detail, icon: Icon, tone = "amber" }: { label: string; value: string; detail: string; icon: typeof Building2; tone?: "amber" | "sky" | "green" | "slate" }) {
  const colors = { amber: "bg-amber-50 text-amber-700", sky: "bg-sky-50 text-sky-700", green: "bg-emerald-50 text-emerald-700", slate: "bg-slate-100 text-slate-600" };
  return <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,.04)]"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[.16em] text-slate-500">{label}</p><p className="mt-3 text-2xl font-bold tracking-tight text-slate-950">{value}</p></div><span className={`grid h-10 w-10 place-items-center rounded-xl ${colors[tone]}`}><Icon className="h-5 w-5" /></span></div><p className="mt-3 text-xs text-slate-500">{detail}</p></section>;
}
...
export default function ConstructionDashboard({ userName = "there", onCreate }: { userName?: string; onCreate?: () => void }) {
  ...
  return <div className="mx-auto max-w-[1540px] space-y-6">
    <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-amber-700">Construction operations center</p><h1 className="mt-2 text-3xl font-bold tracking-tight text-slate-950">Good day, {userName.split(" ")[0]}.</h1><p className="mt-2 text-sm text-slate-500">Live delivery, cost, material, and site coordination for your accessible portfolio.</p></div>{onCreate ? <button onClick={onCreate} className="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-500 px-4 py-2.5 text-sm font-semibold text-slate-950 shadow-sm transition hover:bg-amber-400"><Plus className="h-4 w-4" />New project</button> : <Link href="/projects/new" className="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-500 px-4 py-2.5 text-sm font-semibold text-slate-950 shadow-sm transition hover:bg-amber-400"><Plus className="h-4 w-4" />New project</Link>}</header>
