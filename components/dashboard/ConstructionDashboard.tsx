...
export default function ConstructionDashboard({ onCreate }: { onCreate?: () => void }) {
  const { user } = useAuth();
  const canCreateProject = hasPermission(user, "PROJECT_CREATE");
  ...
  if (error) return <section className="rounded-2xl border border-rose-200 bg-rose-50 p-6 text-rose-800"><h1 className="font-semibold">Dashboard data is unavailable</h1><p className="mt-1 text-sm">{error}</p><button type="button" onClick={() => { setData(null); setError(""); setReloadKey((key) => key + 1); }} className="mt-4 rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white">Retry</button></section>;
